	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action2 !=null) {
			if(action2.equals("rechercher")) {
				...
				List<Emprunt> liste = dao.getAllEmpruntByMC(mc);
				request.setAttribute("emprunts" , liste);
				request.getRequestDispatcher("FindAllEmprunts.jsp").forward(request , response);
			}
			else if(action2.equals("Supprimer")) {
				int id =Integer.parseInt(request.getParameter("id"));
				dao.supprimerEmprunt(id);
				
				List<Emprunt> liste = dao.getAllEmprunt();
				request.setAttribute("emprunts" , liste);
				request.getRequestDispatcher("FindAllEmprunts.jsp").forward(request , response);
			}
